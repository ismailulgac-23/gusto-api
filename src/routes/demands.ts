...
router.post(
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      // Sağlayıcı bildirimi YALNIZCA talep yayına alındıysa gönderilir.
      // Normal kullanıcıların talebi admin onayı bekler (isApproved=false);
      // bildirim, admin onayladığı anda (routes/admin.ts) gönderilir.
      if (autoApprove) {
